...
def demo_image():
    ...
    if not os.path.exists(balls):
        ...
        ugh = urllib.request.urlopen('http://lobstertech.com/media/img/balls.png')
...
def full_chart():
    ...
    for z1, z2 in zip((0, 2, 4), (1, 3, 5)):
        for y1, y2 in zip(list(range(6)), list(range(6))):
            for x1, x2 in zip(list(range(6)), list(range(6))):
